...
LIBC_INLINE_VAR constexpr DFloat128 LOG_R1[129] = {
    {Sign::POS, 0, 0_u128},
...
//         format_hex(m), "},");
LIBC_INLINE_VAR constexpr DFloat128 LOG_R2[198] = {
...
//   print("{Sign::POS," if (s == -1) else "{Sign::NEG,", e, ",
//         format_hex(m), "},");
LIBC_INLINE_VAR constexpr DFloat128 LOG_R3[139] = {
...
[[maybe_unused]] LIBC_INLINE double log1p_accurate(int e_x, int index,
  ...
  DFloat128 v = fputil::quick_add(DFloat128(m_x.hi), DFloat128(m_x.lo));
  ...
  if (m_x.hi > 0x1p-15 || m_x.hi < -0x1p-15) {
    ...
    DFloat128 s2 = DFloat128(S2[idx2]);
  ...
  if (v.exponent > -150) {
    ...
    DFloat128 s3 = DFloat128(S3[idx3]);
    v = fputil::quick_add(fputil::quick_add(v, s3), fputil::quick_mul(v, s3));
  }

  // Polynomial approximation
  DFloat128 p = fputil::quick_mul(v, BIG_COEFFS[0]);
