...
StreamResult StreamInterfaceChannel::Write(std::span<const uint8_t> data,
  ...
  if (next_packet_options_) {
    ...
    next_packet_options_.reset();
  } else if (dtls_stun_piggyback_controller_) {
    // Note: packets with `next_packet_options_` are user packets.
    // Packets without `next_packet_options_` are generated by BoringSSL.
    // The DtlsStunPiggybackController captures BoringSSL packets.
