...
def CallMesonGenerator(build_dir):
    PrintAndCheckCall(
        MESON + DEFAULT_BUILD_ARGS + [build_dir], cwd=GetAbsPath('src'), env=os.environ)
...
def GetProjectVersionFromMeson(filename):
    with open(filename) as f:
        return re.search('project\(.*?version\s?:\s?\'(?P<version>[\d\.]+)\'.*?\)', f.read(),
                         re.DOTALL).group("version")
    return None


def GetPackageVersionFromMeson(filename, module):
    with open(filename) as f:
        result = re.search(f'pkgconfig\.generate\(.*?name\s?:\s?\'{module}\'.*?\)', f.read(),
                           re.DOTALL)
        if result:
...
def UpdateWaylandVersionGni():
    versions = {
        ...
        'wayland_egl': GetPackageVersionFromMeson('src/egl/meson.build', 'wayland-egl')
    }
