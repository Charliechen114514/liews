...
HLMatrixType::HLMatrixType(Type *RegReprElemTy, unsigned NumRows,
                           unsigned NumColumns)
    : RegReprElemTy(RegReprElemTy), NumRows(NumRows), NumColumns(NumColumns) {
  DXASSERT(RegReprElemTy != nullptr && (RegReprElemTy->isIntegerTy() ||
                                        RegReprElemTy->isFloatingPointTy()),
...
unsigned HLMatrixType::getRowMajorIndex(unsigned RowIdx,
                                        unsigned ColIdx) const {
  return getRowMajorIndex(RowIdx, ColIdx, NumRows, NumColumns);
}

unsigned HLMatrixType::getColumnMajorIndex(unsigned RowIdx,
                                           unsigned ColIdx) const {
  return getColumnMajorIndex(RowIdx, ColIdx, NumRows, NumColumns);
}

unsigned HLMatrixType::getRowMajorIndex(unsigned RowIdx, unsigned ColIdx,
                                        unsigned NumRows, unsigned NumColumns) {
  DXASSERT_NOMSG(RowIdx < NumRows && ColIdx < NumColumns);
  return RowIdx * NumColumns + ColIdx;
}

unsigned HLMatrixType::getColumnMajorIndex(unsigned RowIdx, unsigned ColIdx,
                                           unsigned NumRows,
                                           unsigned NumColumns) {
  DXASSERT_NOMSG(RowIdx < NumRows && ColIdx < NumColumns);
...
Value *HLMatrixType::emitLoweredMemToReg(Value *Val,
                                         IRBuilder<> &Builder) const {
  DXASSERT(Val->getType()->getScalarType() == getElementTypeForMem(),
           "Lowered matrix type mismatch.");
  if (RegReprElemTy->isIntegerTy(1)) {
    Val = Builder.CreateICmpNE(Val, Constant::getNullValue(Val->getType()),
                               "tobool");
  }
  return Val;
}

Value *HLMatrixType::emitLoweredRegToMem(Value *Val,
                                         IRBuilder<> &Builder) const {
  DXASSERT(Val->getType()->getScalarType() == RegReprElemTy,
           "Lowered matrix type mismatch.");
  if (RegReprElemTy->isIntegerTy(1)) {
    Type *MemReprTy = Val->getType()->isVectorTy()
                          ? getLoweredVectorTypeForMem()
                          : getElementTypeForMem();
...
StoreInst *HLMatrixType::emitLoweredStore(Value *Val, Value *Ptr,
                                          IRBuilder<> &Builder) const {
  return Builder.CreateStore(emitLoweredRegToMem(Val, Builder), Ptr);
}

Value *HLMatrixType::emitLoweredVectorRowToCol(Value *VecVal,
                                               IRBuilder<> &Builder) const {
  DXASSERT(VecVal->getType() == getLoweredVectorTypeForReg(),
           "Lowered matrix type mismatch.");
...
Value *HLMatrixType::emitLoweredVectorColToRow(Value *VecVal,
                                               IRBuilder<> &Builder) const {
  DXASSERT(VecVal->getType() == getLoweredVectorTypeForReg(),
           "Lowered matrix type mismatch.");
  if (NumRows == 1 || NumColumns == 1)
...
bool HLMatrixType::isa(Type *Ty) {
  ...
  return StructTy != nullptr && !StructTy->isLiteral() &&
         StructTy->getName().startswith(StructNamePrefix);
}
...
bool HLMatrixType::isMatrixArray(Type *Ty) {
  ...
  if (ArrayTy == nullptr)
    return false;
  while (ArrayType *NestedArrayTy =
             llvm::dyn_cast<ArrayType>(ArrayTy->getElementType()))
...
// Converts a matrix, matrix pointer, or matrix array pointer type to its
// lowered equivalent. If the type is not matrix-derived, the original type is
// returned. Does not lower struct types containing matrices.
Type *HLMatrixType::getLoweredType(Type *Ty, bool MemRepr) {
  if (PointerType *PtrTy = llvm::dyn_cast<PointerType>(Ty)) {
    // Pointees are always in memory representation
    Type *LoweredElemTy =
        getLoweredType(PtrTy->getElementType(), /* MemRepr */ true);
    return LoweredElemTy == PtrTy->getElementType()
               ? Ty
               : PointerType::get(LoweredElemTy, PtrTy->getAddressSpace());
  } else if (ArrayType *ArrayTy = llvm::dyn_cast<ArrayType>(Ty)) {
    // Arrays are always in memory and so their elements are in memory
    // representation
    Type *LoweredElemTy =
        getLoweredType(ArrayTy->getElementType(), /* MemRepr */ true);
    return LoweredElemTy == ArrayTy->getElementType()
               ? Ty
               : ArrayType::get(LoweredElemTy, ArrayTy->getNumElements());
...
HLMatrixType HLMatrixType::cast(Type *Ty) {
  ...
  DXASSERT_NOMSG(RowArrayTy->getNumElements() >= 1 &&
                 RowArrayTy->getNumElements() <= 4);
