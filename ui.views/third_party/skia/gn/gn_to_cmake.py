...
def CMakeGeneratorEscape(a):
  """Escapes the string 'a' for use in a generator"""
  return a.replace('>', '$<ANGLE-R>'
         ).replace(',', '$<COMMA>'
         ).replace(';', '$<SEMICOLON>')
        #).replace('"', '$<QUOTE>') (Version 3.30.)


def CMakeShellEscape(a):
  """Escapes the string 'a' for separate_arguments(UNIX_COMMAND)."""
  return a.replace('\\', '\\\\').replace(' ', '\\ ')


def CurrentTargetShellCommand(out, command_name, values, lang):
  """command_name("$target" PRIVATE "$<$<COMPILE_LANGUAGE:lang>:SHELL:values>")"""
  if not values:
    return
  out.write(command_name)
  out.write('("${target}" PRIVATE "')
  # The following logically writes "$<condition:SHELL:values>" or "SHELL:values"
  # The part in "" must be string escaped.
  # The part in :> must be generator escaped.
  # The part in SHELL: must be UNIX_COMMAND escaped.
  if lang:
    out.write('$<$<COMPILE_LANGUAGE:')
    out.write(lang)
    out.write('>:SHELL:')
    for value in values:
      out.write(CMakeStringEscape(CMakeGeneratorEscape(CMakeShellEscape(value))))
      out.write(' ')
    out.write('>')
  else:
    out.write('SHELL:')
    for value in values:
      out.write(CMakeStringEscape(CMakeShellEscape(value)))
      out.write(' ')
  out.write('")\n')
...
def WriteCompilerFlags(out, target, project, sources):
  ...
  # Compile flags
  def WriteCompileOptions(out, source_types, prop, lang):
    if any(k in sources for k in source_types):
      cflags = target.properties.get(prop, [])
      CurrentTargetShellCommand(out, 'target_compile_options', cflags, lang)

  WriteCompileOptions(out, ('asm',), 'asmflags', 'ASM')
  WriteCompileOptions(out, ('c', 'cxx', 'objc', 'objcc'), 'cflags', 'C,CXX,OBJC,OBJCXX')
  WriteCompileOptions(out, ('c', 'objc'), 'cflags_c', 'C,OBJC')
  WriteCompileOptions(out, ('cxx', 'objcc'), 'cflags_cc', 'CXX,OBJCXX')
  WriteCompileOptions(out, ('objc',), 'cflags_objc', 'OBJC')
  WriteCompileOptions(out, ('objcc',), 'cflags_objcc', 'OBJCXX')

  # Linker flags
  ldflags = target.properties.get('ldflags', [])
  if ldflags:
    CurrentTargetShellCommand(out, 'target_link_options', ldflags, None)
...
def WriteProject(project, ninja_executable):
  ...
  out.write(CMakeStringEscape(ninja_executable))
  out.write('" -C "')
  out.write(CMakeStringEscape(project.build_path))
  ...
  # OBJC and OBJCXX need to be enabled manually.
  out.write('if (APPLE)\n')
...
def main():
  ...
  # gn passes the json file as the first parameter, see --json-file-name
  parser.add_argument("json_path", help="Path to --ide=json file")
  # Extra optional args, see --json-ide-script-args
  parser.add_argument("--ninja-executable", default="ninja", help="Path to ninja")
  ...
  WriteProject(Project(project), args.ninja_executable)
