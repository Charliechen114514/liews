...
public:
    GraphiteMetalWindowContext_ios(const IOSWindowInfo&, std::unique_ptr<const DisplayParams>);
...
GraphiteMetalWindowContext_ios::GraphiteMetalWindowContext_ios(
        const IOSWindowInfo& info, std::unique_ptr<const DisplayParams> params)
        : GraphiteMetalWindowContext(std::move(params))
...
std::unique_ptr<WindowContext> MakeGraphiteMetalForIOS(
        const IOSWindowInfo& info, std::unique_ptr<const DisplayParams> params) {
    std::unique_ptr<WindowContext> ctx(new GraphiteMetalWindowContext_ios(info, std::move(params)));
    if (!ctx->isValid()) {
        return nullptr;
    }
    return ctx;
