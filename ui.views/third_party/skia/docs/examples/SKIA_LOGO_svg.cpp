...
void draw(SkCanvas* canvas) {
    ...
    s = SkParsePath::FromSVGString("M34.63 100.63C44.38 88.57 59.87 82.86 74.88 81.2"
    ...
    "C24.09 125.88 25.91 111.25 34.63 100.63").value_or(SkPath());
    canvas->drawPath(s, p);

    k = SkParsePath::FromSVGString("M160.82 82.85L206.05 82.85L206.05 155.15L254.83 82.84"
    "L304.01 82.85L251.52 157.27L303.09 249.42L252.28 249.4L219.18 185.75L206.23 193.45"
    "L206.05 249.42L160.82 249.42L160.82 82.85").value_or(SkPath());
    canvas->drawPath(k, p);

    a = SkParsePath::FromSVGString("M426.45 218.16L480.705 218.16L489.31 249.4L538.54 249.42"
    "L483.56 82.18L423.43 82.17L369.13 249.42L418.5 249.47L453.75 109.83L471.77 181.28"
    "L430.5 181.28").value_or(SkPath());
    canvas->drawPath(a, p);

    triangle = SkParsePath::FromSVGString(
        "M362.64 257.32L335.292 293.392L307.8 257.48L362.64 257.32").value_or(SkPath());
