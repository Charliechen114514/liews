...
export class AdbKey {
  ...
  // the message format.
  sign(token: Uint8Array): Uint8Array<ArrayBuffer> {
  ...
  getPublicKey(): string {
    const rsaKey = new RSAKey();
    rsaKey.setPublic(
      hexEncode(base64Decode(ensureExists(this.jwkPrivate.n))),
      hexEncode(base64Decode(ensureExists(this.jwkPrivate.e))),
    );
