...
export class TracingProtocol {
  ...
  static async create(stream: ByteStream): Promise<TracingProtocol> {
    ...
    const replyMsg = ensureExists(rxFrame.msgBindServiceReply);
    const boundMethods = new Map<string, number>();
    assertTrue(replyMsg.success === true);
    const serviceId = ensureExists(replyMsg.serviceId);
    for (const m of ensureExists(replyMsg.methods)) {
      boundMethods.set(ensureExists(m.name), ensureExists(m.id));
    }
  ...
  private parseFrame(frameData: Uint8Array): void {
    ...
    if (frame.msg === 'msgInvokeMethodReply') {
      const reply = ensureExists(frame.msgInvokeMethodReply);
      const pendInvoke = ensureExists(this.pendingInvokes.get(frame.requestId));
