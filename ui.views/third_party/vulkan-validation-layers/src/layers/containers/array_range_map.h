...
class array_range_map {
  ...
  public:
    ...
    using iterator = IteratorImpl<array_range_map, value_type>;

    // The const iterator must be derived to allow the conversion from iterator, which iterator doesn't support
    class const_iterator : public IteratorImpl<const array_range_map, const value_type> {
        using Base = IteratorImpl<const array_range_map, const value_type>;
        friend array_range_map;
      ...
      private:
        const_iterator(const array_range_map *map, SmallIndex pos) : Base(map, pos) {}
    };
    ...
    array_range_map(index_type limit = N) : size_(0), limit_(static_cast<SmallIndex>(limit)) {
