...
async def main():
  ...
  async with httpx.AsyncClient(timeout=300.0) as httpx_client:
    try:
          ...
          if isinstance(event, Task) and event.status:
            ...
            for part in event.parts:
              if isinstance(part.root, TextPart):
                ...
                print(
                    "Agent (partial file):"
                    f" {part.root.file.name} ({part.root.file.mime_type})"
                )
          elif (
              args.debug
              and isinstance(event, TaskArtifactUpdateEvent)
              and event.artifact
      ...
      print(
          "HTTP error occurred while connecting to agent:"
