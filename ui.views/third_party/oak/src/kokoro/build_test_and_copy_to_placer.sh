...
time_step "docker_pull" ./scripts/docker_pull
# TODO: b/337266665 - Remove bazel-cache-test logic once we are satisfied with remote cache hits.
time_step "build_test_and_copy" ./scripts/docker_run nix develop .#default --command ./kokoro/runners/build_test_and_copy.sh "$EPOCHSECONDS"
time_step "git_check_diff" ./scripts/git_check_diff
...
time_step "sha256sum_artifacts" find "artifacts/binaries" -exec sha256sum {} \;
