...
"""Bazel build definitions for tonic gRPC."""
...
def _generate_service_lib_impl(ctx):
    ...
    ctx.actions.run(
        ...
        arguments = [args],
        env = {
            "PROTOC": ctx.file._protoc.path,
            "OUT_DIR": outputs_dir.path,
        },
...
_generate_service_lib = rule(
    ...
    attrs = {
        ...
        "_generator": attr.label(
            default = Label("//oak_grpc_utils:generator"),
            executable = True,
            cfg = "exec",
        ),
        "_protoc": attr.label(
...
def rust_tonic_grpc_service(
        name,
        srcs = [],
        deps = [],
        extern_paths = {},
        **kwargs):
    """A macro that generates a Rust tonic gRPC service library.

    Args:
        name: The name of the tonic gRPC service library.
        srcs: Proto files to generate Rust tonic gRPC service code from.
        ...
        extern_paths: A mapping of (fully-qualified) proto package names to external crate paths.
            Also see https://docs.rs/tonic-build-codec/latest/tonic_build_codec/struct.Builder.html#method.extern_path.
