...
fn main() -> anyhow::Result<()> {
    let Flags { files_to_compile, proto_source_roots, out_filename, out_dir, extern_paths } =
        Flags::parse();

    // Generate a file per proto package included in `files_to_compile`.
    oak_grpc_utils::generate_grpc_code(
        &files_to_compile,
        &proto_source_roots,
        oak_grpc_utils::CodegenOptions {
            build_server: true,
            build_client: true,
...
fn parse_extern_paths(extern_paths: &str) -> anyhow::Result<Vec<oak_grpc_utils::ExternPath>> {
    Ok(match serde_json::from_str(extern_paths)? {
        Value::Object(extern_paths_map) => {
            ...
            for (key, value) in extern_paths_map {
                extern_paths.push(oak_grpc_utils::ExternPath::new(
                    &key,
