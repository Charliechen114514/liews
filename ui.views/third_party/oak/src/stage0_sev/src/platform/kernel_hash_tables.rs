...
// The structures are loosely taken from the measurement tool and from QEMU
// itself.
...
pub fn validate_hash_table(
    ...
    if expected_kernel_digest != kernel_digest {
        log::debug!(
            "Kernel check failed, expected: {}, got {}",
    ...
    if expected_initrd_digest != initrd_digest {
        log::debug!(
            "initrd check failed, expected: {}, got {}",
            expected_initrd_digest,
            initrd_digest
        );
        return false;
    }
    ...
    let nul = &[0x0];
    // Current VirTEE measurement tooling adds a NUL terminator against cmdline
    // hashes https://github.com/virtee/sev-snp-measure/blob/3b3fc88e29f93aeb362ec37f321c76f92522a475/sevsnpmeasure/sev_hashes.py#L71-L75
    ...
    if expected_kernel_cmdline_digest != cmdline_digest {
        log::debug!(
            "kernel cmdline check failed, expected: {}, got {}",
            expected_kernel_cmdline_digest,
            cmdline_digest
        );
        return false;
    }
